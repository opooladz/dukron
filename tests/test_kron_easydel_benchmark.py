"""
Simple speed test for kron with 1B+ model.
"""
...
def benchmark_1b():
    ...
    logger = ed.utils.get_logger("Kron-1B")
    ...
    logger.info("Initializing KRON...")
    opt_state = tx_kron.init(params)
    logger.info("✓ KRON initialized successfully!")
    ...
    for i in range(3):
        start = time.time()
        updates, opt_state = tx_kron.update(grads, opt_state, params)
    ...
    logger.info(f"\nComparison:")
    if speedup > 1:
        logger.info(f"  Kron is {speedup:.2f}x faster than Adam")
    else:
        logger.info(f"  Adam is {1/speedup:.2f}x faster than Kron")
    
    logger.info(f"\n✅ Benchmark complete!")
    logger.info(f"✅ Kron works with 1B+ model on TPU with FSDP!")
...
if __name__ == "__main__":
    print("Running 1B model benchmark for KRON...")
    result = benchmark_1b()
    print(f"\nResult: {result}")
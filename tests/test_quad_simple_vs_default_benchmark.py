"""
Benchmark comparing default quad vs simple sharding quad.
"""
...
ray.init(runtime_env={
    'py_modules': ['/home/omead/dukron/dukron']
})
...
def benchmark_sharding():
    """Benchmark default quad vs simple sharding quad."""
    import time
    import jax
    ...
    logger = ed.utils.get_logger("QuadSharding")
    ...
    with mesh:
        ...
        # Test 1: Default quad (infers sharding from params)
        logger.info("\n=== TEST 1: DEFAULT QUAD ===")
        logger.info("Using default quad with sharding inferred from parameters...")
        ...
        logger.info("Initializing default QUAD...")
        opt_state_default = tx_default.init(params)
        logger.info("✓ Default QUAD initialized!")
        ...
        # Test 2: Simple sharding quad (first-axis only)
        logger.info("\n=== TEST 2: SIMPLE SHARDING QUAD ===")
        logger.info("Using quad_simple_sharding - automatically shards Q on first axis when size >= 512...")
        
        tx_simple = quad_simple_sharding(
            learning_rate=1e-4,
            b1=0.9,
            weight_decay=0.01,
            preconditioner_lr=0.1,
        ...
        logger.info("Initializing simple sharding QUAD...")
        opt_state_simple = tx_simple.init(params)
        logger.info("✓ Simple sharding QUAD initialized!")
        ...
        # Test 3: Manually specified first-axis sharding
        logger.info("\n=== TEST 3: MANUAL FIRST-AXIS SHARDING ===")
        logger.info("Using regular quad with PartitionSpec('dp', None)...")
        ...
        logger.info("Initializing manually sharded QUAD...")
        opt_state_manual = tx_manual.init(params)
        logger.info("✓ Manually sharded QUAD initialized!")
        
        # Warmup
        logger.info("Warmup (3 steps)...")
        for i in range(3):
            start = time.time()
            updates, opt_state_manual = tx_manual.update(grads, opt_state_manual, params)
            jax.block_until_ready(updates)
            warmup_time = time.time() - start
            logger.info(f"  Warmup {i+1}: {warmup_time:.3f}s")
        ...
        for i in range(10):
            ...
            jax.block_until_ready(updates)
            step_time = time.time() - start
            manual_times.append(step_time)
            if i % 5 == 0:
                logger.info(f"  Step {i}: {step_time:.3f}s")
        
        manual_avg = sum(manual_times) / len(manual_times)
        results["manual"] = manual_avg
        
        # Test Adam for baseline
        logger.info("\n=== ADAM BASELINE ===")
        ...
        logger.info(f"  Default quad:              {results['default']:.4f}s")
        logger.info(f"  Simple sharding quad:      {results['simple']:.4f}s")
        logger.info(f"  Manual PartitionSpec:      {results['manual']:.4f}s")
        logger.info(f"  Adam (baseline):           {results['adam']:.4f}s")
        
        logger.info(f"\nSpeedup vs default:")
        logger.info(f"  Simple sharding: {results['default']/results['simple']:.2f}x")
        logger.info(f"  Manual sharding: {results['default']/results['manual']:.2f}x")
        
        logger.info(f"\nOverhead vs Adam:")
        logger.info(f"  Default:         {results['default']/results['adam']:.2f}x slower")
        logger.info(f"  Simple sharding: {results['simple']/results['adam']:.2f}x slower")
        logger.info(f"  Manual sharding: {results['manual']/results['adam']:.2f}x slower")
        
        # Find best approach
        quad_results = {k: v for k, v in results.items() if k != "adam"}
        best_approach = min(quad_results.items(), key=lambda x: x[1])
        logger.info(f"\n✅ Best approach: {best_approach[0]} ({best_approach[1]:.4f}s per step)")
...
if __name__ == "__main__":
    print("Running quad sharding comparison...")
